"""A small ReAct-style tutoring agent, re-derived here (task-5 branches off
main, which has no earlier task merged into it yet, so each task folder is
self-contained rather than importing across branches) specifically to give
Task 5 something with a real multi-step trajectory and real tool calls to
trace: this is the same three-tool shape as Task 1, kept intentionally
simple since the point of this task is the tracing/eval layer wrapped
around it (see `tracing.py`), not the agent loop itself.
"""
...
def run(question: str, chat_fn=None, max_steps: int = MAX_STEPS) -> dict:
    """Run the ReAct loop, returning the full trajectory for tracing/eval.

    trajectory: list of {type: "action"|"observation"|"error"|"final", ...}
    so tracing.py can turn each entry into a Langfuse span and eval.py can
    inspect exactly which tools were called, in what order, and whether
    any tool call errored.
    """
    ...
    for step in range(max_steps):
        ...
        if action_match:
            ...
            # Neither a parseable action nor a final answer, nudge and retry.
